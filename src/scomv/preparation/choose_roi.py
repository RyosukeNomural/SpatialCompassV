...
def extract_roi(
    ...
    # grid.shortest is indexed by grid-relative (0-based) bin coordinates, while
    # grid.obs["imagecol"/"imagerow"] are absolute µm coordinates. Subtract the
    # grid origin (grid_xedges[0]/grid_yedges[0]) before binning, otherwise
    # xy_bins is offset from grid.shortest.index by floor(origin / bin_size) and
    # filtered_shortest silently selects the wrong grid cells whenever the grid
    # origin isn't at (0, 0) (e.g. any non-Xenium-tutorial dataset).
    x0 = grid.uns["grid_xedges"][0]
    y0 = grid.uns["grid_yedges"][0]
    xy_bins = subset_grid.obs.apply(
        lambda r: (int((r["imagecol"] - x0) // bin_size), int((r["imagerow"] - y0) // bin_size)),
        axis=1
    ).tolist()
...
def _xy_from_shortest_index(
    ...
    # bin -> µm
    x = xb * bin_size + adata.obs.imagecol.min()
    y = yb * bin_size + adata.obs.imagerow.min()

    m = (min_x < x) & (x < max_x) & (min_y < y) & (y < max_y)
    x = x[m]
    y = y[m]

    # Re-derive grid-relative bins from the original (xb, yb) instead of
    # floor-dividing the absolute µm coordinates again — that round-trip drops
    # the grid origin and silently returns bins shifted from shortest_df.index
    # by floor(imagecol.min() / bin_size) whenever the origin isn't at 0.
    gx = xb[m].tolist()
    gy = yb[m].tolist()
    return x, y, gx, gy
...
def contour_regions(
    ...
    def xy_from_df(df):
        ...
        m = (min_x < x) & (x < max_x) & (min_y < y) & (y < max_y)
        x, y = x[m], y[m]
        # Re-derive grid-relative bins from (xb, yb) directly instead of
        # floor-dividing the absolute µm coordinates again (see
        # _xy_from_shortest_index above for why the round-trip is unsafe).
        gx = xb[m].tolist()
        gy = yb[m].tolist()
        return x, y, gx, gy
